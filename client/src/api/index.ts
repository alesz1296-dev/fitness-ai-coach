...
export const searchApi = {
  ...
  exercises: (
    q: string,
    opts: { muscle?: string; equipment?: string; difficulty?: string } = {},
    limit = 25,
  ) => {
    const params = new URLSearchParams({ q, limit: String(limit) });
...
export const workoutsApi = {
  ...
  startFromTemplate: (templateId: number, date?: string, name?: string) =>
    api.post<{ workout: Workout; template: Partial<WorkoutTemplate> }>(
      `/workouts/start-from-template/${templateId}`,
      { name, date },
    ),
  getExerciseProgression: (name: string, limit = 30) =>
    api.get<{
      ...
      allTimePR: ExerciseProgression | null;
  ...
  updateExercise: (
    exerciseId: number,
    data: Partial<WorkoutExerciseUpdateInput>,
  ...
  addExercise: (
    ...
    api.post<{ exercise: WorkoutExercise }>(
...
export const templatesApi = {
  getAll: (filters?: {
    ...
    api.get<{ templates: WorkoutTemplate[] }>("/templates", {
      params: filters,
    }),
  getRecommended: (filters?: {
    ...
    api.get<{
      grouped: Record<string, WorkoutTemplate[]>;
      all: WorkoutTemplate[];
    }>("/templates/recommended", { params: filters }),
  getOne: (id: number) =>
    api.get<{ template: WorkoutTemplate }>(`/templates/${id}`),
  create: (data: any) =>
    api.post<{ template: WorkoutTemplate }>("/templates", data),
  update: (id: number, data: any) =>
    api.put<{ template: WorkoutTemplate }>(`/templates/${id}`, data),
  delete: (id: number) => api.delete(`/templates/${id}`),
  addExercise: (id: number, data: any) =>
    api.post(`/templates/${id}/exercises`, data),
...
export const foodApi = {
  getToday: (date?: string) =>
    api.get<{ logs: FoodLog[]; totals: FoodTotals; date: string }>(
      `/foods${date ? `?date=${date}` : ""}`,
    ),
  getHistory: (days = 7) =>
    api.get<{ history: Record<string, FoodTotals>; days: number }>(
      `/foods/history?days=${days}`,
    ),
  ...
  frequent: (limit = 5) =>
    ...
    }>(`/foods/frequent?limit=${limit}`),
  log: (data: Partial<FoodLog>) => api.post<{ log: FoodLog }>("/foods", data),
  bulk: (foods: Array<Partial<FoodLog>>, date?: string) =>
    api.post<{ logs: FoodLog[]; message: string }>("/foods/bulk", {
      foods,
      date,
    }),
  update: (id: number, data: Partial<FoodLog>) =>
    api.put<{ log: FoodLog }>(`/foods/${id}`, data),
...
export const weightApi = {
  getLogs: (days = 30) =>
    api.get<{ logs: WeightLog[]; stats: WeightStats | null; days: number }>(
      `/weight?days=${days}`,
    ),
...
export const calorieGoalsApi = {
  ...
  getActive: () =>
    api.get<{ goal: CalorieGoal | null }>("/calorie-goals/active"),
  create: (data: any) =>
    api.post<{ goal: CalorieGoal; calculation: any; warning?: string }>(
      "/calorie-goals",
      data,
    ),
  preview: (data: any) =>
    api.post<{ calculation: any; projection: any[] }>(
  ...
  getProjection: (id: number) =>
    api.get<{
      ...
      actual: any[];
      summary: any;
    }>(`/calorie-goals/${id}/projection`),
  update: (id: number, data: any) =>
    api.put<{ goal: CalorieGoal }>(`/calorie-goals/${id}`, data),
...
export const chatApi = {
  ...
  getHistory: (agentType?: string, page = 1, limit = 20) =>
    api.get<{ conversations: Conversation[]; total: number }>(
      `/chat/history?page=${page}&limit=${limit}${agentType ? `&agentType=${agentType}` : ""}`,
    ),
...
export const weeklyPlanApi = {
  get: (week?: string) =>
    api.get<{ plan: WeeklyPlan | null; weekStart: string }>(
      `/weekly-plan${week ? `?week=${week}` : ""}`,
    ),
  upsert: (data: {
    week?: string;
    days: Array<{
  ...
  toggleDay: (
    dayId: number,
    data?: { actualCalories?: number; workoutId?: number },
  ) =>
    api.patch<{ day: WeeklyPlanDay }>(
      `/weekly-plan/days/${dayId}/toggle`,
      data ?? {},
    ),
...
export const waterApi = {
  ...
  getHistory: (days = 7) =>
    api.get<{ history: Record<string, number>; days: number }>(
...
export const mealPlansApi = {
  ...
  addEntry: (planId: number, dayId: number, data: Partial<MealPlanEntry>) =>
    api.post<{ entry: MealPlanEntry }>(
      `/meal-plans/${planId}/days/${dayId}/entries`,
      data,
    ),
...
export const calendarApi = {
  getMonth: (month: string) =>
    api.get<{ days: WorkoutCalendarDay[]; month: string }>(
