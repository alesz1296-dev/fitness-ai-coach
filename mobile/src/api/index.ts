// Mobile API — identical to the web version.
// The only difference is the axios instance import (mobile uses SecureStore).
...
interface WorkoutCreateInput {
  ...
  caloriesBurned?: number; notes?: string; templateId?: number;
  trainingType?: string; exercises?: WorkoutExerciseCreateInput[];
}
...
export const searchApi = {
  foods: (q: string, limit = 20, tag?: string) => {
    ...
    return api.get<{ results: any[]; total: number }>(`/search/foods?${params}`);
  },
...
export const workoutsApi = {
  getAll:  (page = 1, limit = 20) => api.get<{ workouts: Workout[]; total: number; page: number; pages: number }>(`/workouts?page=${page}&limit=${limit}`),
  ...
  update:  (id: number, data: Partial<Workout>) => api.put<{ workout: Workout }>(`/workouts/${id}`, data),
  ...
  deleteExercise: (exerciseId: number) => api.delete(`/workouts/exercises/${exerciseId}`),
  addExercise:    (workoutId: number, data: { exerciseName: string; sets: number; reps: number; weight?: number | null; rpe?: number | null; notes?: string }) =>
...
export const foodApi = {
  getToday:   (date?: string) => api.get<{ logs: FoodLog[]; totals: FoodTotals; date: string }>(`/foods${date ? `?date=${date}` : ""}`),
  getHistory: (days = 7)      => api.get<{ history: Record<string, FoodTotals>; days: number }>(`/foods/history?days=${days}`),
  frequent:   (limit = 5)     => api.get<{ frequent: any[] }>(`/foods/frequent?limit=${limit}`),
  log:        (data: Partial<FoodLog>)                    => api.post<{ log: FoodLog }>("/foods", data),
  bulk:       (foods: Array<Partial<FoodLog>>, date?: string) => api.post<{ logs: FoodLog[]; message: string }>("/foods/bulk", { foods, date }),
  update:     (id: number, data: Partial<FoodLog>)        => api.put<{ log: FoodLog }>(`/foods/${id}`, data),
  delete:     (id: number)    => api.delete(`/foods/${id}`),
};

export const weightApi = {
  getLogs: (days = 30) => api.get<{ logs: WeightLog[]; stats: WeightStats | null; days: number }>(`/weight?days=${days}`),
  log:     (data: { weight: number; notes?: string; date?: string }) => api.post<{ log: WeightLog }>("/weight", data),
...
export const chatApi = {
  send: (data: { message: string; agentType?: "coach" | "nutritionist" | "general" }) =>
    api.post<{
      message: string; agentType: string; conversationId: number; tokensUsed?: number;
      suggestedWorkout?: Record<string, any>; suggestedPlan?: Record<string, any>; suggestedMealPlan?: Record<string, any>;
    }>("/chat", data),
  ...
  clearHistory: (agentType?: string) => api.delete(`/chat/history${agentType ? `?agentType=${agentType}` : ""}`),
  saveWorkout:      (data: any) => api.post("/chat/save-workout", data),
  saveCaloriePlan:  (data: any) => api.post("/chat/save-calorie-plan", data),
};

export const waterApi = {
  log:        (amount: number, date?: string) => api.post<{ log: WaterLog }>("/water", { amount, date }),
  getToday:   (date?: string) => api.get<{ logs: WaterLog[]; totalMl: number; targetMl: number; date: string }>(`/water/today${date ? `?date=${date}` : ""}`),
  getHistory: (days = 7)      => api.get<{ history: Record<string, number>; days: number }>(`/water/history?days=${days}`),
...
export const reportsApi = {
  ...
  getOne:   (year: number, month: number)                                             => api.get<{ report: MonthlyReport }>(`/reports/${year}/${month}`),
  generate: (data?: { year?: number; month?: number; aiSummary?: boolean })           => api.post<{ report: MonthlyReport }>("/reports/generate", data),
};

export const weeklyPlanApi = {
  get:       (week?: string) => api.get<{ plan: WeeklyPlan | null; weekStart: string }>(`/weekly-plan${week ? `?week=${week}` : ""}`),
  ...
  deletePlan:(planId: number) => api.delete(`/weekly-plan/${planId}`),
};

export const calendarApi = {
  getMonth:  (month: string) => api.get<{ days: WorkoutCalendarDay[]; month: string }>(`/calendar?month=${month}`),
  populate:  (data: { month: string; assignments: Array<{ dayOfWeek: number; workoutName: string; muscleGroups?: string[]; templateId?: number; isRestDay?: boolean }>; overwrite?: boolean }) =>
    api.post<{ message: string; count: number }>("/calendar/populate", data),
  updateDay: (date: string, data: { workoutName?: string | null; muscleGroups?: string[] | null; templateId?: number | null; isRestDay?: boolean; notes?: string | null }) =>
    api.put<{ day: WorkoutCalendarDay }>(`/calendar/${date}`, data),
  deleteDay: (date: string) => api.delete(`/calendar/${date}`),
  swap:      (date1: string, date2: string) => api.post<{ message: string }>("/calendar/swap", { date1, date2 }),
  clearMonth:(month: string) => api.delete<{ message: string; count: number }>(`/calendar/clear?month=${month}`),
};

export const mealPlansApi = {
  getAll:         ()                                           => api.get<{ plans: MealPlan[] }>("/meal-plans"),
  getOne:         (id: number)                                 => api.get<{ plan: MealPlan }>(`/meal-plans/${id}`),
  create:         (data: { name: string; weekStart: string })  => api.post<{ plan: MealPlan }>("/meal-plans", data),
  update:         (id: number, data: any)                      => api.put<{ plan: MealPlan }>(`/meal-plans/${id}`, data),
  ...
  deleteEntry:    (planId: number, entryId: number)            => api.delete(`/meal-plans/${planId}/entries/${entryId}`),
