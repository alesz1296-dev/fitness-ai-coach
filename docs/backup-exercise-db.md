# Exercise Database Backup

**105 items** — last entry `e114`. Source: `src/data/exercises.ts`.
This file is the reference copy of `EXERCISE_DB` kept for token-safe context recovery.
The canonical runtime source is now the `ExerciseItem` table (seeded from this array).

## Muscle groups
`Chest` `Back` `Shoulders` `Biceps` `Triceps` `Forearms` `Legs` `Quads`
`Hamstrings` `Glutes` `Calves` `Core` `Traps` `Full Body` `Cardio` `Stretching`
