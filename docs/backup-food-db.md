# Food Database Backup

**366 items** — last entry `f409`. Source: `src/data/foods.ts`.
This file is the reference copy of `FOOD_DB` kept for token-safe context recovery.
The canonical runtime source is now the `FoodItem` table (seeded from this array).

## Tags in use
`keto` `fit` `high-protein` `vegan` `vegetarian` `integral` `low-carb` `fruit`
`vegetable` `dairy` `legume` `mix` `meat` `seafood` `cheese` `soup` `sausage`
`pasta` `japanese` `italian` `mexican` `middle-eastern` `korean` `smoothie`
`high-fat` `high-sugar` `dessert` `fast-food`
