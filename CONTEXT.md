...
- The regular-user coach invite-code flow is now easier to discover on the dashboard: the client join form moved out of the mixed proposal card and into its own dedicated end-of-dashboard coach-connect section, while the old slot only leaves a small directional hint.
- That coach-connect section now also has a visible header-level `Have a coach code?` shortcut plus an extra helper shortcut, both of which scroll and focus the code input, and successful linking now raises the top coach-updates area, shows a dedicated success card, and leaves a small persistent connected badge on the dashboard header.
- The regular-user dashboard now also shows a temporary coach-connect onboarding banner, a colored CTA card directly under the greeting, and a sticky mobile shortcut until a coach is linked, so the code-entry path is now visible in both desktop and mobile scanning patterns.
