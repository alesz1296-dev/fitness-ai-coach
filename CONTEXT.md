...
- That coach-connect section now also has a small top-of-dashboard shortcut that scrolls and focuses the code input, plus a dedicated success card after invite acceptance so the user gets a clearer connected state and a direct path back to coach updates.
